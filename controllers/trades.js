...
exports.addTrades = async (req, res) => {
    ...
    try {
        ...
        if (data) {
            return res.status(200).json({success: true, message: ` Trades Added`, data});
      ...
      console.error(error);
        res.status(500).json({ message: `Error adding Trades` });
    }
  }
  ...
  exports.getTrades = async (req, res) => {
    ...
    if (data.length > 0) {
       res.status(200).json({ success: true, message: `Transactions statistics`, data });
        //res.json(data[0]); 
    } else {
        ...
        console.error(error);
        res.status(500).json({ message: `Error Fectching Data` });
    }
};